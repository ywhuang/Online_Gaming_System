package edu.columbia.gamesys.server;
/*****
...
import edu.columbia.gamesys.game.Game;
import edu.columbia.gamesys.game.Player;
import edu.columbia.gamesys.helper.ST;
            ...
            switch (swS) {
			 case 1: //login
				...
				if (ipCountDataTree.get(clientRealIPString) > 5 ) {
				 ...
				 if (st.contains(userName)){
					 // do nothing if the user already existed
					ackStatus = "F" ; // login fail
			 ...
			 case 2://query list - [ list,<Id>,<userName> ]
			  ...
			  case 31:
				  ...
				  newplayer1.setOpponentport(newplayer2.getPlayerport());				  				
				  newplayer2.setOpponentport(newplayer1.getPlayerport());
				  ...
				  System.out.println("check player1 name:"+ newGame.getPlayer1().getName() );
				  System.out.println("check player2 name:"+ newGame.getPlayer2().getName() );
				  
				  System.out.println("check player1 port:"+ newGame.getPlayer1().getPlayerport() );
				  System.out.println("check player2 port:"+ newGame.getPlayer2().getPlayerport() );
			  ...
			  case 4:  //play     receive paket contet: play,<packetID>,<user>,<positioin number>
				  ...
				  String player1Name = currentGame.getPlayer1().getName();
				  String player2Name = currentGame.getPlayer2().getName();
				  System.out.println("currentGame.player1.name:"+ player1Name );
				  System.out.println("currentGame.player2.name:"+ player2Name );
				  //check if current player is 1 or 2
				  if (user.equals(player1Name ) ){
					  
					  currentPlayer = currentGame.getPlayer1();
					  System.out.println(user + " is player 1");
				  } else if (user.equals(player2Name) ){
				  ...
				  //check turns
				  if (currentGame.getTurn() == currentPlayer.getPlayerNumber() ) {	
				  ...
				  //Check if the positoin is oppupied
				  if (currentGame.getJudgeView().contains(position)) {
				  ...
				  //Check win
					  ...
					  if (currentGame.getPlayer1().equals(currentPlayer) ) {
						  ...
						  targetport = currentGame.getPlayer2().getPlayerport(); // msg targets to opponent
						  //check winning move?
						  if (currentGame.checkWin(currentGame.getPlayer1Record())) {
							  // winning
							  System.out.println("player 1 -"+currentPlayer.getName() +" won");
						  ...
						  targetport = currentGame.getPlayer1().getPlayerport(); // msg targets to opponent
						  
						 //check if winning move
						  if (currentGame.checkWin(currentGame.getPlayer2Record())) {
							  System.out.println("player 2 -"+currentPlayer.getName() +" won");
							  //currentGame.gameOver = true;
							  //currentGame.switchTurn();
							  //inform winner
							  int targetportTemp = portDataTree.get(userName);
					  ...
					  if (currentGame.isFull()) {
                          ...
                          //inform opponent						  
                          targetport = currentPlayer.getOpponentport();
					  ...
					  currentGame.switchTurn();
					  System.out.println("switch turn to " + currentGame.getTurn() );
					  } 
